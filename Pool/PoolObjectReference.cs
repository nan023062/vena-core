...
public readonly struct Ref<T> : IEquatable<Ref<T>> where T : class, IPoolable, new()
{
    ...
    /// </summary>
    public readonly PoolObjectToken token;
    ...
    {
        if (wref.token &&  wref._impl != null )
        {
            PoolObjectToken token = PoolManager.GetToken(wref._impl);
    ...
    where T : class, IPoolable<TArg>, new()
{
    ...
    public static implicit operator bool(Ref<T, TArg> wref)
    {
        ...
        if (wref.token &&  wref._impl != null )
        {
            PoolObjectToken token = PoolManager.GetToken<T, TArg>(wref._impl);
