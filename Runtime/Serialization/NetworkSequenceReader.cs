...
namespace Vena
{
    public ref partial struct NetworkSequenceReader<T> where T : unmanaged, IEquatable<T>
    {
        /// <summary>
        ...
        public NetworkSequenceReader(in ReadOnlySequence<T> sequence)
        {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public NetworkSequenceReader(ReadOnlyMemory<T> memory)
        {
        ...
        internal bool TryCopyMultisegment(Span<T> destination)
        {
            ...
            while (this.Sequence.TryGet(ref next, out ReadOnlyMemory<T> nextSegment, true))
            {
                if (nextSegment.Length > 0)
                {
                    ...
                    int toCopy = System.Math.Min(nextSpan.Length, destination.Length - copied);
